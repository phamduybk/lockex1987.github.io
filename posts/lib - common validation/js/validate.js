 ...
 * Kiểm tra rỗng, 0 không coi là rỗng.
 * Chú ý: nếu có nhập số 0 sẽ trả về false.
 * @param val Giá trị.
...
function normalizeInputField(item) {
	// Nếu là file thì bỏ qua việc chuẩn hóa input
	if (item.is("[type=file]")) {
		return;
	}
...
function validateType(item, value) {
	...
	if (type === 'email' || item.attr('type') === "email") {
		var regExp = /(^[a-z]([a-z0-9_\.]*)@([a-z_\.]*)([.][a-z]{3})$)|(^[a-z]([a-z0-9_\.]*)@([a-z_\.]*)(\.[a-z]{3})(\.[a-z]{2})*$)/i;
		if (!regExp.test(value)) {
			return tranlateErrorMessage('Định dạng email không hợp lệ'); // Invalid email
		}
	}

	if (type === 'phone' || item.attr('type') === "tel") {
		// Tu 9 den 12 ky tu
		const minLength = 9;
		const maxLength = 12;
		if (value.length < minLength || value.length > maxLength) {
			return 'Số ký tự có độ dài từ {0} đến {1} ký tự. Số ký tự bạn nhập vào là {2}'.format(minLength, maxLength, value.length);
		}
	...
	if (type === 'integer') {
		var regExp = /(^-?\d\d*$)/;
		if (!regExp.test(value)) {
			return tranlateErrorMessage('Số nguyên không hợp lệ'); // Invalid integer
		}
	}
	...
	if (type === 'number') {
		...
		if (isNaN(value)) {
			return tranlateErrorMessage('Không phải là số'); // Not a number
		}
	}
...
function validateRange(item, value) {
	...
	if (minValue) {
		if (parseFloat(minValue) > parseFloat(value)) {
			return tranlateErrorMessage("Giá trị nhỏ nhất là {0}").format(minValue); // Min value is
		}
	}
	...
	if (maxValue) {
		if (parseFloat(maxValue) < parseFloat(value)) {
			return tranlateErrorMessage("Giá trị lớn nhất là {0}").format(minValue); // Max value is
		}
	}
...
function validateRequired(item, value) {
	...
	if (required) {
		if (!value) {
			...
			var msg = item.attr('data-required-error-message') ||  "Please enter this field";
