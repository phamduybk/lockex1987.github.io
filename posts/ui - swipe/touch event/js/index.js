...
function handleStart(evt) {
    ...
    for (var i = 0; i < touches.length; i++) {
        if (touches[i].clientX - offset.x > 0 &&
            ...
            ctx.beginPath();
            ctx.arc(touches[i].clientX - offset.x,
                    touches[i].clientY - offset.y, 4, 0, 2 * Math.PI, false); // a circle at the start
            ctx.fillStyle = color;
            ctx.fill();
            //log("touchstart:" + i + ".");
        }
